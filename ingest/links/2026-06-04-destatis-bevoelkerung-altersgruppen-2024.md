# Ingest: Destatis Bevölkerung nach Altersgruppen bis 2024

## Metadaten

- Typ: Link
- Datum: 2026-06-04
- Quelle: https://www.destatis.de/DE/Themen/Gesellschaft-Umwelt/Bevoelkerung/Bevoelkerungsstand/Tabellen/bevoelkerung-altersgruppen-deutschland-absulut-basis-2022.html
- Status: geprüft
- Index: `ingest/index/README.md`

## Kurzfassung

Destatis-Tabelle zur Bevölkerung Deutschlands nach Altersgruppen. Sie wurde
für den Ausgangsbestand 2024 und die Altersstruktur genutzt. Die Quelle ist
zentral, um Vorausberechnungen mit dem aktuellen Bevölkerungsstand zu
verbinden.

## Kernaussagen

- Amtliche Bestandsdaten nach Altersgruppen.
- Basis für Altersquotienten und Beitragszahler-/Leistungsempfänger-Relationen.
- Für feinere Modelle werden später Einzelalter oder 5-Jahres-Gruppen benötigt.

## Enthaltene Informationen

- Destatis-Bestandsdaten zur Bevölkerung Deutschlands nach Altersgruppen bis
  2024.
- Altersgruppenwerte als Ausgangsbasis für Erwerbsbevölkerung, ältere
  Bevölkerung und Belastungskennzahlen.

## Jetzt extrahierte relevante Informationen

- Die Quelle stützt den Ausgangswert 2024 für Bevölkerung, Altersstruktur und
  Erwerbsalter im Renten- und Demographiemodell.
- Die im Repo genutzten Werte sind in
  `ingest/dokumente/2026-06-04-destatis-demographie.md` und
  `analysen/daten/2026-06-04-demographie-kernzahlen-2024-2070.csv`
  verdichtet.

## Relevanz für agenda2030

Grundlage der demographischen Status-quo-Rechnung.

## Zuordnung

- Ministerien: `ministerien/arbeit-soziales/`, `ministerien/gesundheit/`
- Gesetze/Gesetzbücher: `gesetzbuecher/sgb/`
- Bundestagsdrucksachen: keine
- Themen: Bevölkerung, Altersstruktur, Demographie

## Offene Fragen

- Maschinenlesbare Detaildaten für spätere Modelle ergänzen.

## Nächste Schritte

- In Demographie-Skripten als Bestandsanker verwenden.
